...
The repo containing raw data for this analysis is available to download [here](https://doi.org/10.5525/gla.researchdata.2012).  Relevant collected data is located in subfolder `case_study_1` of the data repository. Curated dataset used for the cul-de-sac analysis is available on this repo. 
...
This represnets a row in the final dataset.  Note that the majority of data collected using the Linux top utility was not used in the analysis.  Where this is the case we have left the data point uncommented.  If this holds for each run, the execute `cul-de-sac/build_dataset.py` to generate the `DATASET.csv` for the analysis. Again this aspect of the process is fully automated, but we suggest checking the data first. 
